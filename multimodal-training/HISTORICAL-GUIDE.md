...
> Historical guide retained from the incremental experiments. Start with the
> [current application README](README.md) and [experiment map](EXPERIMENTS.md).
> Status statements below describe their original slice, not today's full
> repository. Commands and measured pins are retained for reproduction.
