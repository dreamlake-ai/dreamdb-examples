...
def main():
    ...
    report = {"tag": tag, "mode": a.mode, "writers": a.writers, "refs": refs,
              "payload_file_sha256": hashlib.sha256(a.vectors.read_bytes()).hexdigest(),
    ...
    for ref in refs:
        ...
        actual = {}
        for b in ds.iter_all_batches(fields=[FIELD, "payload_sha256"],
                                     start_ns=0, end_ns=a.rows):
            for anchor, value, digest in zip(b["_time_anchors"], b[FIELD], b["payload_sha256"]):
                anchor = int(anchor)
                if anchor in actual:
                    issues.append("duplicate anchor")
                if value is None:
                    issues.append("missing exact vector")
                    continue
                decoded = (np.frombuffer(value, dtype="<f4") if isinstance(value, (bytes, bytearray))
                           else np.asarray(value, dtype="<f4"))
                if not 0 <= anchor < a.rows or not np.array_equal(decoded, vectors[anchor]):
                    issues.append("vector content mismatch")
                elif digest != hashlib.sha256(vectors[anchor].tobytes()).hexdigest():
